...
class AnnealedVAEModel(VAEModel):
    ...
    def update_encoding_capacity(self):
        if self.n_iters > self.config.hyperparameters.c_change_duration:
            ...
            self.C =  min(self.config.hyperparameters.c_min + (self.config.hyperparameters.c_max - self.config.hyperparameters.c_min) * self.n_iters / self.config.hyperparameters.c_change_duration, self.config.hyperparameters.c_max)
...
#                    print(name, '{:0.2f}'.format(param.data.sum().item()))
            ...
            # save losses
            final_losses = {'discriminator': loss_discriminator, 'vae': loss_vae, 'generator': loss_generator, 'total': loss_discriminator+loss_vae+loss_generator}
