...
class SoftDecisionTreeModel(dnn.BaseDNN, gr.BaseModel):
    ...
    def run_training (self, train_loader, n_epochs, valid_loader = None, logger=None):
        ...
        if logger is not None:
            dummy_input = torch.FloatTensor(1, self.config.inner_node.n_channels, self.config.inner_node.input_size[0], self.config.inner_node.input_size[1]).uniform_(0,1)
    ...
    def get_encoder(self):
        return SoftDecisionTreeEncoder(self.network, self.config.leaf_node.network.parameters.n_latents, self.config.tree.max_depth)
...
class SoftDecisionTreeLoss(losses.BaseLoss):
    def __init__(self, leaf_loss_name = "VAE", leaf_loss_parameters = models.VAEModel.default_config().loss.parameters, **kwargs):
    ...
    def __call__(self, x, y, leaf_outputs, path_prob, reduction = True, logger=None, **kwargs):
        if self.leaf_loss_name == "VAE":
            ...
            for k in leaf_losses.keys():
                leaf_losses[k] = leaf_losses[k] * path_prob.squeeze() # we weight the gradient by path_prob to lower the influence of unsure results
...
class SoftDecisionTreeDecoder(decoders.BaseDNNDecoder):
    ...
    def forward(self, z, path):
        decoder_idx = int (path, 2) # the path is given in base 2 and we convert it to integer (respect the order of decoder list)
