...
class DIMLeafNode(LeafNode, models.DIMModel):
    def __init__(self, depth, lf_layers=None, config=None, **kwargs):
        ...
        new_network_parameters = self.config.network.parameters
        new_network_parameters.n_latents *= (self.depth + 1)
        self.network.global_discrim = models.dim.GlobalDiscriminator(new_network_parameters.n_latents,
                                                                     self.network.encoder.local_feature_shape)
        self.network.local_discrim = models.dim.LocalEncodeAndDotDiscriminator(new_network_parameters.n_latents,
                                                                               self.network.encoder.local_feature_shape)
...
class HierarchicalTreeModel(dnn.BaseDNN, gr.BaseModel):
    ...
    def forward(self, x):
        ...
        while depth < self.config.tree.max_depth:
            ...
            for path in pathes:
                ...
                cur_node_x_ids = [i for i, x_path in enumerate(tree_path_taken[:, :depth + 1]) if
                                  torch.all(torch.eq(x_path, path))]
        ...
        if self.config.leaf_node_classname == "VAE":
            ...
            model_outputs.update(
                {"global_pos": global_pos, "global_neg": global_neg, "local_pos": local_pos, "local_neg": local_neg,
    ...
    def forward_for_graph_tracing(self, x):
        ...
        for path in pathes:
            ...
            if self.config.leaf_node_classname == "VAE":
                ...
                cur_global_pred, cur_local_pred, cur_prior_pred = cur_leaf.forward_for_graph_tracing(x[cur_leaf_x_ids],
                                                                                                     tree_z[
                                                                                                         cur_leaf_x_ids])
                if leaf_output is None:
                ...
                cur_prob_pos, cur_recon_x = cur_leaf.forward_for_graph_tracing(x[cur_leaf_x_ids],
                                                                               tree_z[cur_leaf_x_ids])
                if leaf_output is None:
    ...
    def get_encoder(self):
        return HierarchicalTreeEncoder(self.network, self.network.config.network.parameters.n_latents,
        ...
        for path in pathes:
            ...
            for k in leaf_losses.keys():
                ...
                leaf_losses[k] = leaf_losses[
                    k]  # path_prob[node_x_ids].squeeze() # we weight the gradient by path_prob to lower the influence of unsure results
...
class HierarchicalTreeDecoder(decoders.BaseDNNDecoder):
    ...
    def forward(self, z, path_taken):
        ...
        pathes = torch.unique(path_taken, dim=0)
        for path in pathes:
            cur_decoder_idx = int(tensor2string(path),
