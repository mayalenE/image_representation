...
def plot_barbox_per_datasource(data=None, experiment_ids=None, data_source=None, repetition_ids=None, config=None,
                               **kwargs):
    default_config = dict(
    ...
    config = goalrepresent.config.set_default_config(kwargs, config, default_config)

    # load data
    plot_data, experiment_ids = goalrepresent.gui.jupyter.misc.get_datasource_data(data=data,
    ...
    for datasource_idx, cur_data_source in enumerate(data_source):
        ...
        trace_label = goalrepresent.gui.jupyter.misc.replace_str_from_dict(str(trace_label),
    ...
    for experiment_idx, experiment_id in enumerate(experiment_ids):
        ...
        group_label = goalrepresent.gui.jupyter.misc.replace_str_from_dict(str(group_label), {'exp_idx': experiment_idx,
                                                                                              'exp_id': experiment_id})
        new_group_labels.append(group_label)
    config.group_labels = new_group_labels

    if config.plot_type == 'plotly_meanstd_bar':
        return goalrepresent.gui.jupyter.plotly_meanstd_bar(data=plot_data, config=config)
    elif config.plot_type == 'plotly_box':
        return goalrepresent.gui.jupyter.plotly_box(data=plot_data, config=config)
