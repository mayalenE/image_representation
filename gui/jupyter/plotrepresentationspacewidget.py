...
from goalrepresent.gui.jupyter.misc import create_colormap, transform_image_from_colormap, transform_image_PIL_to_bytes
...
class LatentSpaceViewerWidget(ipywidgets.Box):
    ...
    def get_default_gui_config():
        ...
        default_config.canvas_data = gr.Config()
        ...
        default_config.image_list_widget = gr.gui.jupyter.ImageListWidget.get_default_gui_config()
    ...
    def __init__(self, database=None, config=None, **kwargs):
        self.config = gr.config.set_default_config(kwargs, config, LatentSpaceViewerWidget.get_default_gui_config())
        ...
        ## Row 2: 
        self.image_list_widget = gr.gui.jupyter.ImageListWidget(config=self.config.image_list_widget)
    ...
    def update(self, database):
        ...
        if self.config.transform_images:
            for item_id, item in self.database.items():
                item['image'] = transform_image_PIL_to_bytes(
                    transform_image_from_colormap(item['image'], self.config.transform_images_colormaps[0]))
    ...
    def selection_fn(self, trace, points, selector):
        ...
        if trace.visible:
            self.displayed_data['images'] += [
                self.database[self.database_filtered_ids[trace.name][relative_point_id]]['image'] for relative_point_id
                in points.point_inds]
            self.displayed_data['captions'] += [
                '{}, id: {}'.format(self.database[self.database_filtered_ids[trace.name][relative_point_id]]['label'],
                                    self.database_filtered_ids[trace.name][relative_point_id]) for relative_point_id in
                points.point_inds]
...
class PlotRepresentationSpaceWidget(ipywidgets.Box):
    ...
    def get_default_gui_config():
        ...
        default_config.exp_rep_selection_widget = gr.gui.jupyter.ExperimentRepetitionLoaderWidget.get_default_gui_config()

        # Row 1: Latent Space viewer widget
        default_config.latent_space_viewer_widget = gr.gui.jupyter.plotrepresentationspacewidget.LatentSpaceViewerWidget.get_default_gui_config()

        return default_config

    def __init__(self, experiment_definitions, repetition_ids, pca_data, tsne_data, umap_data,
                 experiment_statistics=None, filters=None, config=None, **kwargs):
        # set config with priority for args in kwards -> config -> default_config
        self.config = gr.config.set_default_config(kwargs, config,
                                                   PlotRepresentationSpaceWidget.get_default_gui_config())
        ...
        ## Row 0
        self.exp_rep_selection_widget = gr.gui.jupyter.ExperimentRepetitionLoaderWidget(
    ...
    def load_database(self, experiment_id, repetition_id):
        ...
        if not dir_content.__len__() == self.database.__len__():
            warnings.warn(
                'The number of final observation runs from {!r} differs from the number of representation runs from {!r} '.format(
                    final_observation_path, representation_path))
        ...
        for curr_filter_k, curr_filter_val in self.filters.items():
            ...
            if curr_filter_val is None or not curr_filter_val:
                ...
                run_ids = gr.gui.jupyter.misc.filter_single_experiment_data(self.experiment_statistics[experiment_id],
                                                                            curr_filter_val, int(repetition_id))
