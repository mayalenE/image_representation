...
class ImageListWidget(ipywidgets.VBox):
    ...
    def get_default_gui_config():
        ...
        default_config.output_layout = gr.Config()
        ...
        default_config.content_ouput = gr.Config()
        default_config.page_label = gr.Config()

        default_config.page_selection = gr.Config()
        default_config.page_selection.description = 'Page: '

        default_config.previous_page_button = gr.Config()
        default_config.previous_page_button.description = '<'
        default_config.previous_page_button.layout = gr.Config()
        default_config.previous_page_button.layout.width = '20px'

        default_config.next_page_button = gr.Config()
        default_config.next_page_button.description = '>'
        default_config.next_page_button.layout = gr.Config()
        default_config.next_page_button.layout.width = '20px'

        default_config.button_box = gr.Config()
        default_config.button_box.layout = gr.Config()
        ...
        default_config.image_items = gr.Config()
        default_config.image_items.layout = gr.Config()
        ...
        default_config.image_captions = gr.Config()

        return default_config

    def __init__(self, images=None, config=None, **kwargs):

        self.config = gr.config.set_default_config(kwargs, config, ImageListWidget.get_default_gui_config())
